...
export function extractMessagesFromHtml(html: string) {
  ...
  if ((trimmed.startsWith("{") && trimmed.endsWith("}")) || (trimmed.startsWith("[") && trimmed.endsWith("]"))) {
    try {
      ...
      // Case 1: Already an array of messages
      if (Array.isArray(data) && data.length > 0 && data[0].role) {
        return { title: "Extracted Chat", messages: data, isDeadLink: false };
      }
      
      // Case 2: ChatGPT specific JSON structure
      if (data.mapping && data.current_node) {
        ...
        if (messages.length > 0) {
          return { title: data.title || "Extracted Chat", messages, isDeadLink: false };
        }
      }
    } catch (e) {
      // Not JSON or failed to parse, continue to HTML parsing
    }
  }
